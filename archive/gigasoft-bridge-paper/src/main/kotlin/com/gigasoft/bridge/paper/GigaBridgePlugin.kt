...
class GigaBridgePlugin : JavaPlugin() {
    ...
    override fun onEnable() {
        ...
        runtime = GigaRuntime(
            ...
            hostAccess = PaperHostBridge(server).asHostAccess()
        ) { msg -> logger.info(msg) }
    ...
    private fun adapterSecurityConfig(): AdapterSecurityConfig {
        val section = config.getConfigurationSection("adapter_security")
        return AdapterSecurityConfig(
            maxPayloadEntries = section?.getInt("max_payload_entries", 32) ?: 32,
            maxPayloadKeyChars = section?.getInt("max_payload_key_chars", 64) ?: 64,
            maxPayloadValueChars = section?.getInt("max_payload_value_chars", 512) ?: 512,
            maxPayloadTotalChars = section?.getInt("max_payload_total_chars", 4096) ?: 4096,
            maxCallsPerMinute = section?.getInt("max_calls_per_minute", 180) ?: 180,
            invocationTimeoutMillis = section?.getLong("invocation_timeout_millis", 250L) ?: 250L
        )
    }
