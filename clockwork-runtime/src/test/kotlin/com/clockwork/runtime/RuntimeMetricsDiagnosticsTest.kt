...
class RuntimeMetricsDiagnosticsTest {
    ...
    fun `profile marks adapter hotspots when timeout rate is high`() {
        ...
        assertEquals(25L, profile.adapterCounters.total)
        assertEquals(24L, profile.adapterCounters.accepted)
        assertEquals(1L, profile.adapterCounters.timeouts)
        assertTrue(profile.adapterAudit.totalRecorded >= 25L)
    }
