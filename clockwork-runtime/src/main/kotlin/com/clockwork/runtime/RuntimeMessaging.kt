...
import com.clockwork.api.CommandRegistry
import com.clockwork.api.EventBus
import com.clockwork.api.EventPriority
import com.clockwork.api.EventSubscriptionOptions
import com.clockwork.api.EventTraceSnapshot
import com.clockwork.api.EventTypeTraceSnapshot
...
import com.clockwork.api.defaultCompletions
import com.clockwork.api.hasPermission
import com.clockwork.api.helpText
import com.clockwork.api.render
...
class RuntimeCommandRegistry(
    ...
    private data class RegisteredCommand(
        ...
        val action: (com.clockwork.api.CommandInvocationContext) -> com.clockwork.api.CommandResult,
    ...
    private class MutableCommandTelemetry {
        ...
        fun record(result: com.clockwork.api.CommandResult, durationNanos: Long) {
        ...
        fun snapshot(command: String): com.clockwork.api.CommandTelemetrySnapshot {
            ...
            val topErrors = errorCounts.entries
                .map { com.clockwork.api.CommandErrorCount(it.key, it.value.get()) }
                .sortedByDescending { it.count }
                .take(5)
            return com.clockwork.api.CommandTelemetrySnapshot(
                command = command,
                totalRuns = total,
                failures = failed,
    ...
    override fun registerSpec(
        ...
        val profile = policy ?: com.clockwork.api.CommandPolicyProfiles.forPlugin(pluginId)
        ...
        val completionProvider = completion ?: com.clockwork.api.CommandCompletionContract { _, _, commandSpec, args ->
            commandSpec.defaultCompletions(args)
        }
        com.clockwork.api.CommandCompletionCatalog.register(
            command = key,
    ...
    override fun registeredCommands(): Map<String, com.clockwork.api.CommandSpec> {
        return handlers.mapValues { it.value.spec }
    }

    override fun commandTelemetry(commandOrAlias: String): com.clockwork.api.CommandTelemetrySnapshot? {
        val key = resolve(commandOrAlias) ?: return null
        val metric = telemetry[key] ?: return null
        return metric.snapshot(command = key)
    }

    override fun commandTelemetry(): Map<String, com.clockwork.api.CommandTelemetrySnapshot> {
    ...
    override fun execute(ctx: PluginContext, sender: com.clockwork.api.CommandSender, commandLine: String): String {
        ...
        val route = com.clockwork.api.resolveCommandRoute(handler.spec, args)
        ...
        if (pre.cancelled) {
            val response = pre.overrideResponse ?: com.clockwork.api.CommandResult.error(
        ...
        return try {
            if (routedArgs.isNotEmpty() && (routedArgs[0].equals("help", ignoreCase = true) || routedArgs[0] == "--help")) {
                val help = com.clockwork.api.CommandResult.ok(routedSpec.helpText())
                val duration = System.nanoTime() - started
            ...
            val parsedResult = com.clockwork.api.parseCommandArgs(routedSpec, routedArgs)
            ...
            if (requiredPermission != null && !ctx.hasPermission(requiredPermission)) {
                val denied = com.clockwork.api.CommandResult.error(
            ...
            if (routedSpec.rateLimitPerMinute > 0) {
                ...
                synchronized(queue) {
                    ...
                    if (queue.size >= routedSpec.rateLimitPerMinute) {
                        val limited = com.clockwork.api.CommandResult.error(
            ...
            if (routedSpec.cooldownMillis > 0L) {
                ...
                if (last != null && now - last < routedSpec.cooldownMillis) {
                    ...
                    val cooling = com.clockwork.api.CommandResult.error(
            ...
            val invocation = com.clockwork.api.CommandInvocationContext(
                pluginContext = ctx,
                sender = sender,
                rawArgs = routedArgs,
                spec = routedSpec,
            ...
            val orderedMiddleware = handler.middleware.sortedWith(
                compareBy<com.clockwork.api.CommandMiddlewareBinding> { it.phase.ordinal }
                    .thenBy { it.order }
                    .thenBy { it.id }
            )
            var index = -1
            fun executeNext(): com.clockwork.api.CommandResult {
                index++
            ...
            val response = com.clockwork.api.CommandResult.error(
