...
import com.clockwork.api.GigaAdapterPostInvokeEvent
import com.clockwork.api.GigaAdapterPreInvokeEvent
...
data class AdapterInvocationTelemetry(
    val adapterId: String,
    val action: String,
    val outcome: AdapterInvocationOutcome,
    val detail: String,
...
class RuntimeModAdapterRegistry(
    ...
    private val invocationObserver: (AdapterInvocationTelemetry) -> Unit = { }
) : ModAdapterRegistry {
    ...
    private val invokeExecutor = if (fastMode) {
        ...
        Executors.newFixedThreadPool(
            ...
            Thread(runnable, "clockwork-adapter-$pluginId-${threadCounter.incrementAndGet()}").apply { isDaemon = true }
        }
    }
    @Volatile
    ...
    override fun invoke(adapterId: String, invocation: AdapterInvocation): AdapterResponse {
        ...
        if (pre.cancelled) {
            ...
            observe(adapterId, invocation, AdapterInvocationOutcome.DENIED, response.message ?: "Adapter invocation cancelled", started)
        ...
        pre.overrideResponse?.let { response ->
            observe(adapterId, invocation, AdapterInvocationOutcome.ACCEPTED, "Adapter invocation overridden by pre event", started)
        ...
        if (fastMode || invocationTimeoutMillis <= 0L) {
            return try {
                ...
                observe(adapterId, invocation, AdapterInvocationOutcome.ACCEPTED, "Adapter invocation accepted", started)
                ...
                observe(adapterId, invocation, AdapterInvocationOutcome.FAILED, response.message.orEmpty(), started)
        ...
        return try {
            ...
            observe(adapterId, invocation, AdapterInvocationOutcome.TIMEOUT, response.message.orEmpty(), started)
    ...
    private fun denied(adapterId: String, invocation: AdapterInvocation, message: String, started: Long): AdapterResponse {
        observe(adapterId, invocation, AdapterInvocationOutcome.DENIED, message, started)
    ...
    private fun observe(
        ...
        invocationObserver(
            AdapterInvocationTelemetry(
                adapterId = adapterId,
                action = invocation.action,
                outcome = outcome,
                detail = detail,
                durationNanos = System.nanoTime() - started,
                payloadEntries = invocation.payload.size
            )
        )
    }
